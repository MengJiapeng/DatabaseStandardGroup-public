...
| score\_rule\_id | bigint unsigned | YES |   | 评分规则 |
...
| gmt\_create | datetime | YES |   | 表示记录创建时间 |
| gmt\_modified | datetime | YES |   | 表示最近修改时间 |
| report | tinyint unsigned | YES |   | 报告占期末比例 |
...
| attendance\_status | tinyint unsigned | YES |   | 点名状态 |
...
| course\_id | bigint unsigned | YES |   | 所属课程 |
| is\_fixed | tinyint unsigned | YES |   | 分组方式 |
| start\_time | date | NO |   |   |
| end\_time | date | NO |   |   |
...
| leader | bigint unsigned | YES |   | 组长编号 |
...
| student\_id | bigint unsigned | YES |   |   |
| is\_leader | tinyint unsigned | YES |   | 是否为队长 |
...
| gmt\_modified | datetime | YES |   | 表示最近修改时间 |
| student\_id | bigint unsigned | YES |   |   |
| seminar\_group\_topic | bigint unsigned | YES |   | 课堂小组话题 |
